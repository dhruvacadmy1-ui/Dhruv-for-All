...
function showScore() {
    ...
    let playerClass = localStorage.getItem('playerClass') || '';
    ...
    let list = leaderboard.map((s, idx) => {
        let medal = idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : '✨';
        return `<li>${medal} ${s.name}: ${s.score}/15</li>`;
    }).join("");

    document.getElementById("game-container").innerHTML = `
        <h1>🏆 Final Score Card</h1>
        <div style="font-size: 24px; margin: 20px 0;">
            <strong>${playerName}</strong>, you scored <br>
...
function goHome() {
    window.location.href = '/Dhruv-for-All/';
}
</script>
